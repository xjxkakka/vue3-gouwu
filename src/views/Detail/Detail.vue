<template>
  ...
  <div class="detail">
    ...
    <section class="con">
      ...
      <div class="conPoin">
        <!--        显示多级分类-->
        <!--        返回的是个对象，所以用v-show来判断有没category1Name 有就显示-->
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :skuImageList="skuImageList1"></Zoom>
          <!--          小图列表-->
          <image-list :skuImageList="skuImageList1"></image-list>
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
...
export default {
  ...
  setup() {
    ...
    //vuex发起请求不能获得参数，参考这里使用 获取computed的值一定要加 .value!!!!!
    //给子组件数据，不要直接skuInfo.xxx，传计算属性给他
    let skuImageList1 = computed(() => {
      //至少给个空数组不能让他为undefined
    ...
    //产品售卖属性切换高亮，使用排他，获取当前数组和所有数组
    //saleAttrValue当前点击的数组， arr表示所有数组
    let changActive = (saleAttrValue, arr) => {
      //遍历所有数组
      arr.forEach(item => {
        item.isChecked = 0;
        // console.log(arr,'所有数组')
      })
      //把当前数组的变为高亮
      saleAttrValue.isChecked = 1;
      // console.log(saleAttrValue,'当前数组')
    }

    //商品数量增加减少
    let skuNum = ref(1);
    let changeSkuNum = (event) => {
      //用户输入进来的文本 * 1
    ...
    //购物车的回调函数
    let addShopCar = async ()=>{
      //1.发请求 ---将产品加入到数据库(通知服务器)
      /*
      * 当前这里派发一个action，向服务器发请求，判断加入购物车是成功还是失败了，进行相应操作
      * store.dispatch('addOrUpdateShopCart',{skuId:route.params.skuId,skuNum:skuNum.value})
        上面这行代码说白了:调用仓库中的addOrUpdateShopCart，这个方法加上了async，返回的一定是Promise
        * 要么成功|要么失败
      * */
      try {
        ...
        //产品信息的数据【比较复杂:skuInfo】，通过会话存储(不持久化，会话数据在消失)
        //本地存储|会话存储，一般存储的是字符串   把skuInfo变成JSON格式传过去，在变回对象
        sessionStorage.setItem('SKUINFO',JSON.stringify(skuInfo.value))
        await router.push({name: 'AddCartSuccess', query: {skuNum: skuNum.value}})
      }catch (error){
        alert(error.message)
      }
    }
    return {categoryView, skuInfo,addShopCar, spuSaleAttrList, skuNum, changeSkuNum, skuImageList1, changActive}
  }
}
</script>
