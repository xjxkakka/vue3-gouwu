  ...
  setup(props, context) {
    ...
    onMounted(async () => {
      ...
      try {
        ...
        await store.dispatch('getOrderInfo')
    ...
    let addressInfo = computed(() => {
      return store.state.trade.address;
    })
    //获取商品清单
    ...
    //用户选择地址
    let changeDefault = (address, addressInfo) => {
    ...
    let submitOrder = async () => {
      ...
      //发送请求，需要带的参数tradeNo,和data
      let result = await reqSubmitOrder(tradeNo, data)
      //提交订单成功
      if (result.code === 200) {
        orderId.value = result.data
        //路由跳转+路由传参
        await router.push(`/pay?orderId=${orderId.value}`);
      } else {
        console.log(result)
      }

    }
    return {msg, addressInfo, orderInfo, userDefaultAddress, submitOrder, changeDefault}
  }
